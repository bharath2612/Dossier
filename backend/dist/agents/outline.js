...
async function generateOutline(enhancedPrompt, researchData) {
    try {
...
Create 8-12 slides with STRONG, SPECIFIC titles (never generic!). Return ONLY valid JSON (no markdown):`;
        ...
        const validTypes = ['intro', 'content', 'data', 'quote', 'conclusion'];
        ...
        if (error instanceof Error && error.message.includes('Invalid JSON')) {
            ...
            try {
                const { content, tokens } = await (0, anthropic_1.callClaude)(SYSTEM_PROMPT + '\n\nCRITICAL: Return ONLY valid JSON. No markdown, no explanations, ONLY JSON.', `Create outline for: "${enhancedPrompt}"\n\nResearch: ${JSON.stringify(researchData, null, 2)}\n\nReturn ONLY JSON:`, {
                    maxTokens: 3000,
                    temperature: 0.5,
                });
